...
@router.get('/')
def get_routines(db: db_dependancy, user: user_dependency):
    return db.query(Routine).options(joinedload(Routine.workouts)).filter(Routine.user_id==user.get('id')).all()

@router.post('/')
def create_routines(db: db_dependancy, user: user_dependency, routine:RoutineCreate):
...
@router.delete('/')
def delete_routine(db: db_dependancy, user: user_dependency, routine_id: int):
